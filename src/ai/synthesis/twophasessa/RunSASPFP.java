...
public class RunSASPFP {
    ...
    public static void main(String[] args) {
        ...
        TradutorDSL tScrTeste = new TradutorDSL("moveaway(Light) train(Heavy,6,EnemyDir) attack(Heavy,lessHealthy) build(Barrack,1,Right) if(HaveUnitsToDistantToEnemy(Light,3)) then(moveaway(Heavy)) else(moveaway(Ranged) moveToUnit(Light,Ally,closest)) attack(Light,weakest) moveaway(Ranged)");
        iDSL ScrTeste = tScrTeste.getAST();
        ...
        String intencao = "moveaway(Light) train(Heavy,6,EnemyDir) attack(Heavy,lessHealthy) build(Barrack,1,Right) if(HaveUnitsToDistantToEnemy(Light,3)) then(moveaway(Heavy)) else(moveaway(Ranged) moveToUnit(Light,Ally,closest)) attack(Light,weakest) moveaway(Ranged)";
        System.out.println("Intenção: ");
        System.out.println(intencao);
        System.out.println("DSL montada: ");
        System.out.println(ScrTeste.translate());
        if (intencao.equals(ScrTeste.translate())) {
